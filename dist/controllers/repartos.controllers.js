...
const listarTodos = async (req, res) => {
    try {
        ...
        const repartosConItems = await Promise.all(repartos.map(async (reparto) => {
...
const getCliente = async (id) => {
    try {
        ...
        if (resultado.length === 1) {
            ...
            const { tipo_doc, documento, nombres, telefono, correo, genero, distrito_id, direc, referencia, url_maps } = resultado[0];
            return {
                tipo_doc,
                documento,
                nombres,
...
exports.default = { listarTodos, getReparto, insertar, darConformidad, actualizar, eliminar };
